...
@app.cell
def _(patterns):
    print([getattr(patterns, g) for g in patterns.ATTRS])
    return
...
def _(np):
    ...
    num_pix_1 = len(coor2d)
    return coor2d, num_pix_1
...
@app.cell
def _(coor2d, ewald_rad, np, num_pix_1):
...
@app.cell
def _(np, num_pix_1):
    factor = np.random.uniform(0.0003, 0.0004, num_pix_1)
    return (factor,)
...
@app.cell
def _(ef, np, num_pix_1, r2d):
    mask = np.zeros(num_pix_1, np.int16)
...
@app.cell
def _(coor, ef, ewald_rad, factor, mask):
    det = ef.detector(
...
@app.cell
def _(det, np):
    print(det[np.random.rand(det.num_pix) < 0.5])
    return
