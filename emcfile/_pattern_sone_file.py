...
def read_patterns(fn: Path, idx_con, ones_idx: np.ndarray, multi_idx: np.ndarray):
    seek_start = (
        PatternsSOneEMC.HEADER_BYTES + I4 * (len(ones_idx) - 1) * 2
    )
...
class PatternsSOneEMC:
    ...
    def __init__(self, fn):
        ...
        self._seek_start = (
            self.HEADER_BYTES + I4 * self.num_data * 2
        )
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, (int, np.integer)):
            ...
            place_ones, place_multi, count_multi = read_patterns(
                self._fn, idx_con, self._ones_idx, self._multi_idx
            )
            ans[place_ones] = 1
            ...
            return PatternsSOne(
                ...
                self.ones[idx],
                self.multi[idx],
                *read_patterns(self._fn, idx_con, self._ones_idx, self._multi_idx),
            )
